...
public class World {
	...
	public boolean isGround (Vector pos) {
		...
		Vector posInRoom = new Vector(pos.x - roomPos.x * Room.GET_TOTAL_SIZE(), pos.y - roomPos.y * Room.GET_TOTAL_SIZE());
	...
	public Vector getRoomPositionAt (Vector pos) {
		pos.multiply(1 / Room.GET_TOTAL_SIZE());
	...
	public void render (GraphicsContext gc) {
		...
		for (int i = 0 ; i < rooms.length ; i++) {
			for (int j = 0 ; j < rooms.length ; j++) {
				gc.save();
				gc.translate(i * Room.GET_TOTAL_SIZE() * Renderer.CELLSIZE, j * Room.GET_TOTAL_SIZE() * Renderer.CELLSIZE);
				rooms[i][j].render(gc);
				gc.restore();
			}
		}
